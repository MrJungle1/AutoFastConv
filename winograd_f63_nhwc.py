...
def winograd():
    ...
    Weight = te.placeholder(
        (kernel_size, kernel_size, Input_Channel, Output_Channel), name="Weight")
    A, B, G = gen_matrices()
    ...
    V_tmp1 = te.compute(
        ...
        lambda h, w, n, k:
            data_pad[n // (nH * nW), n // nW % nH * block_size + h, n %
                     nW * block_size + w, k],
        name="V_tmp1",
    )
    ...
    V = te.compute(
        (tile_size, tile_size, Tile, Input_Channel),
        lambda h, w, n, k: te.sum(V_tmp2[h, ik2, n, k] * B[ik2, w], axis=ik2),
        name="V",
    )

    wk1 = te.reduce_axis((0, kernel_size), "wk1")
    wk2 = te.reduce_axis((0, kernel_size), "wk2")
    U_tmp = te.compute(
        (tile_size, kernel_size, Input_Channel, Output_Channel),
        lambda h, w, k, m: te.sum(
            Weight[wk1, w, k, m] * G[h, wk1],
            axis=wk1
        ),
        name="U_tmp",
    )
    ...
    U = te.compute(
        (tile_size, tile_size, Input_Channel, Output_Channel),
        lambda h, w, n, k: te.sum(U_tmp[h, wk2, n, k] * G[w, wk2], axis=wk2),
        name="U",
    )
    ...
    OT_tmp2 = te.compute(
        (block_size, block_size, Tile, Output_Channel),
        lambda h, w, n, m: te.sum(OT_tmp1[h, ok2, n, m] * A[ok2, w], axis=ok2),
        name="OT_tmp2",
    )
    O = te.compute(
        (N, H, W, Output_Channel),
        lambda n, h, w, m:
            OT_tmp2[h % block_size, w % block_size, n * nH * nW + h // block_size *
                    nW + w // block_size, m],
        name="O",
    )
    return [data, Weight, O]
...
if __name__ == "__main__":
    ...
    b = np.random.rand(kernel_size, kernel_size,
                       Input_Channel, Output_Channel).astype(dtype)

    Weight = tvm.nd.array(b, dev) 
    A = tvm.nd.array(a, dev)
    O = tvm.nd.array(np.zeros((N, H, W, Output_Channel), dtype=dtype), dev)

    cost = evaluator(A, Weight, O).mean
    ...
    for n in range(N):
        for o in range(Output_Channel):
            for i in range(Input_Channel):
                ...
                tmpb = np.zeros((kernel_size, kernel_size), dtype=dtype)
                for h in range(H + 2):
                    for w in range(W + 2):
                        tmpa[h][w] = a[n][h][w][i]
